        ...
        for attempt in range(5):
            try:
                ...
                if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                    ...
                    raise e
        raise Exception("Failed to generate syllabus after 5 retries.")
...
os.environ["GOOGLE_API_KEY"] = envs_dict.get("GOOGLE_API_KEY", envs_dict.get("OPENAI_API_KEY", ""))
...
task_specify_agent = DiscussAgent(
    task_specifier_sys_msg, ChatGroq(model="llama-3.1-8b-instant", temperature=1.0)
)
...
def extract_text(msg):
    ...
    if isinstance(c, list): return "".join(i.get("text", "") if isinstance(i, dict) else str(i) for i in c)
    return str(c)
...
def generate_syllabus(topic, task):
    ...
    specified_task = extract_text(specified_task_msg)
    assistant_sys_msg, user_sys_msg = get_sys_msgs(
        assistant_role_name, user_role_name, specified_task
    )

    assistant_agent = DiscussAgent(
        assistant_sys_msg, ChatGroq(model="llama-3.1-8b-instant", temperature=0.2)
    )
    user_agent = DiscussAgent(user_sys_msg, ChatGroq(model="llama-3.1-8b-instant", temperature=0.2))
    ...
    chat_turn_limit, n = 2, 0  # Reduced to 2 to avoid Google Free Tier Rate Limits
    while n < chat_turn_limit:
        n += 1
        time.sleep(3)  # Respect free-tier rate limits
        user_ai_msg = user_agent.step(assistant_msg)
        user_msg = HumanMessage(content=extract_text(user_ai_msg))
        ...
        assistant_msg = HumanMessage(content=extract_text(assistant_ai_msg))
    ...
    summarizer_agent = DiscussAgent(
        summarizer_sys_msg, ChatGroq(model="llama-3.1-8b-instant", temperature=1.0)
    )
    ...
    return extract_text(summarizered_msg)
